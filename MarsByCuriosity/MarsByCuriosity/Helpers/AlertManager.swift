...
final class AlertManager {
    ...
    static func showAlert(iconName: String?, iconColor: UIColor? = nil, title: String?, message: String?, button: String? = nil, action:(()->())? = nil) {
        shared.showAlert(iconName: iconName, iconColor: iconColor, title: title, message: message, button: button, action: action)
    }
    ...
    static func showAttention(_ message: String?, action:(()->())? = nil) {
        shared.showAlert(iconName: nil, title: shared.attentionTitle, message: message, action: action)
    }
    
    static func showAlert(iconName: String?, iconColor: UIColor? = nil, title: String?, message: String?, buttons:[String], preferredButton:Int? = nil, viewController:UIViewController? = nil, isDismissEqualToCompletion: Bool = true, completion:((Int)->())? = nil) {
        shared.showAlert(iconName: iconName, iconColor: iconColor ,title: title, message: message, buttons: buttons, preferredButton: preferredButton, viewController: viewController, isDismissEqualToCompletion: isDismissEqualToCompletion, completion: completion)
    }
    ...
    func showAlert(iconName: String?, iconColor: UIColor? = nil, title: String?, message: String?, button: String? = nil, action:(()->())? = nil) {
        let buttonTitle = (button ?? "").isEmpty ? self.continueButtonTitle : button!
           
        self.showAlert(iconName: iconName, iconColor: iconColor, title: title, message: message, buttons: [buttonTitle]) { (selectedIndex) in
                action?()
            }
    }
    ...
    func showAttention(_ message: String?, action:(()->())? = nil) {
        self.showAlert(iconName: nil,
                       title: self.attentionTitle,
                       message: message,
                       action: action)
    }
       
    func showAlert(iconName: String?,
